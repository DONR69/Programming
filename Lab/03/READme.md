...
**Постановка задачи:**
1.  Используя стандартный модуль для разработки программ с графическим интерфейсом Tkinter, создать игру "Lines" на языке Python.
2.  В качестве образца графического интерфейса использовать  [эту игру](http://game-shariki.ru/linii-2).
...
### 1. Разработка ПО
Для создания клиентского приложения использовался язык Python, исходный код разделен на несколько модулей. Приложение использует различные библиотеки для работы с графикой, такие как `Tkinter` и `Pillow`. Для запуска игры необходимо открыть python файл `start.py`.
...
Чтобы найти кратчайший путь, приложение использует метод поиска пути в ширину. Алгоритм обходит все свободные ячейки поочередно, делая от каждой пройденной ячейки в очереди один шаг. Это позволяет найти кратчайшее расстояние до необходимой ячейки. Этот алгоритм поиска описан в модуле `game`.
```py
...
![](screenshots/beginning of the game.png)

Рисунок 1 - Начало игры

![](screenshots/deleted first line with blue balls.png)

Рисунок 2 - Игрок удалил первую линию из синих шаров по вертикали, получив за это 10 игровых очков

![](screenshots/game over.png)

Рисунок 3 - Игра окончена, игровое поле заполнено

## Вывод
Я научился разрабатывать игру на языке Python, получил навыки в разработке игр на языке Python. Понял принцип работы и функции библиотек Tkinter и Pillow.